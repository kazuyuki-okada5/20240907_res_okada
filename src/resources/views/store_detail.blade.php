                        ...
                        <div class="review">
                            <div class="star-rating">
                                <div class="star-gloup">
                                    @for ($i = 1; $i <= 5; $i++)
                                        <span class="star" style="color: {{ $reviews->rating >= $i ? 'gold' : 'gray' }}">&#9733;</span>
                                    @endfor
                                </div>
                                <div class="review-actions">
                                    @can('update', $reviews)
                                        <a href="{{ route('reviews.edit', $reviews->id) }}" class="btn btn-primary">口コミを編集</a>
                                    @endcan
                                    @can('delete', $reviews)
                                        <a href="#" class="btn btn-danger" onclick="event.preventDefault(); document.getElementById('delete-form-{{ $reviews->id }}').submit();">口コミを削除</a>
                                        <form id="delete-form-{{ $reviews->id }}" action="{{ route('reviews.destroy', $reviews->id) }}" method="POST" style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    @endcan
                            ...
                            <div class="review-posted">
                                <p>投稿日: {{ $reviews->created_at->format('Y-m-d H:i') }}</p>
                                <p>投稿者: {{ $reviews->user->name }}</p>
                            </div>
                        </div>
                    @else
                        <p>まだ口コミがありません。</p>
